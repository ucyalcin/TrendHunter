...
# Sayfa Ayarları
st.set_page_config(page_title="BAŞKAN TREND HUNTER (WEEKLY)", layout="wide")

# ==========================================
# 1. AYARLAR
# ==========================================
st.sidebar.header("HAFTALIK TARAMA MODU")

# Varsayılan olarak 1 Hafta seçili gelir
tf_label = st.sidebar.selectbox("Zaman Dilimi", ("1 Hafta", "1 Gün"))

tf_map = {
    # HAFTALIK: period="max" ve Pine Motoru ile tam isabet
    "1 Hafta":  {"interval": "1d", "period": "max", "custom_weekly": True}, 
    # GÜNLÜK: Derin analiz için max veri
    "1 Gün":    {"interval": "1d", "period": "max", "custom_weekly": False}, 
}
selected_tf = tf_map[tf_label]

st.sidebar.markdown("---")

# Haftalıkta Ext Hours mantıksızdır, kapalı tutuyoruz.
use_ext_hours = False 
...
freshness = st.sidebar.number_input("Sinyal Tazeliği (Son kaç mum?)", min_value=1, value=3, step=1)
...
start_btn = st.sidebar.button("HAFTALIK TARAMAYI BAŞLAT", type="primary")

# ==========================================
# 2. ÖZEL MUM MİMARI (WEEKLY)
# ==========================================
...
# ==========================================
# 3. HESAPLAMA MOTORU (PINE SCRIPT ENGINE)
# ==========================================
def calculate_rma_pine(series, length):
    """
    PineScript RMA: İlk değer SMA, sonraki değerler recursive.
    """
    series = series.copy()
...
def calculate_supertrend(df, period=10, multiplier=3):
    ...
    start_idx = period  
    # İndeks hatası almamak için kontrol
    if start_idx-1 < len(up_val):
...
def analyze(df, symbol, dema_len, st_atr, st_fact, fresh, adx_len, use_dema, is_debug=False):
    # FİLTRE: 2000 YILI ÖNCESİNİ SİL (Kirli veriyi temizle)
    if df.index.tz is not None:
         cutoff = pd.Timestamp("2000-01-01").tz_localize(df.index.tz)
    else:
    ...
    if len(df) < (dema_len + 50): 
        if is_debug: st.error(f"Yetersiz Veri (2000 sonrası): {len(df)}")
        return None
    ...
    if is_debug:
        st.write(f"### 🧬 {symbol} DETAYLI ANALİZİ (WEEKLY ENGINE)")
...
def fetch_and_analyze(symbol, tf_conf, use_ext, is_debug=False):
    try:
        # WEEKLY SPECIAL olduğu için her zaman 'max' çekiyoruz
        target_interval = '1d' # Haftalık için günlük çekip resample yapacağız
        ...
        # Sadece Haftalık Modu Var (Ya da Günlük Max)
...
# 6. ARAYÜZ
# ==========================================
st.title("🚀 BAŞKAN TREND HUNTER (WEEKLY SPECIAL)")
